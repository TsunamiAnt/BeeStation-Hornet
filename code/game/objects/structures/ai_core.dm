			...
			if(GLASS_CORE)
				...
				if(P.tool_behaviour == TOOL_SCREWDRIVER)
					...
					if(brain)
						...
						deadchat_broadcast(" has been brought online at <b>[get_area_name(A, format_text = TRUE)]</b>.", span_name(A.name), follow_target=A, message_type = DEADCHAT_ANNOUNCEMENT)
						qdel(src)
					else
						state = AI_READY_CORE
						update_icon()
